...
const groupSlice = createSlice({
  ...
  reducers: {
    ...
    addNewMessageInGroups: (state, action) => {
      console.log("ADDING TO  GROUPS");
      return state.map((group) => {
        if (group._id === action.payload.receiverId) {
          return {
            ...group,
            groupMessages: [...(group.groupMessages || []), action.payload.senderId] // Ensure immutability
          };
        }
        return group; // Ensure all groups are returned
      });
    },
...
export const { setGroups, addNewMessageInGroups, addGroup, intializeGroups, removeMemberFromGroups, addMemberInGroups } = groupSlice.actions;
